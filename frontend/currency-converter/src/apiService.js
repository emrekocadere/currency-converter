...
export async function convertCurrencyOnDate(request) {
    ...
    const response = await axios.get('http://localhost:5203/api/CurrencyConverter/exchange/by-date',
             { params: { date: request.date, currencies: request.currencies, amount: request.amount } }
    )
    return response
}
